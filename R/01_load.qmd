...
if (!file.exists(str_c(data_dir, expr_file))) {
  download.file(
    url = data_url,
    destfile = zip_file,
    mode = "wb"
...
expr_data <- read_csv(str_c(data_dir, expr_file))
patient_data <- read_csv(str_c(data_dir, patient_file))

```

```{r}
# Replace spaces with underscores in file names -> we need to chnage this to tidyverse
#  unzipped_files <- list.files(path =  data_dir, full.names = TRUE)
